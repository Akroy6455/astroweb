...
export default function AspectMatrix({ data }: AspectMatrixProps) {
  if (!data || !data.positions || !data.lagna) {
    return <div style={{textAlign: 'center', padding: '2rem'}}>No data available for Aspect Matrix</div>;
  }
  ...
  const getAspectPercentage = (aspector: string, aspectorLong: number, aspectedLong: number) => {
    const a = norm(aspectedLong - aspectorLong);
    const virupas = getAspectVirupas(aspector, a);
    return Math.round((virupas / 60) * 100);
  };
  ...
  rows.forEach(rowPlanet => {
    ...
    PLANETS.forEach(colPlanet => {
      if (rowPlanet !== colPlanet) {
        ...
        const aspectValue = getAspectPercentage(rowPlanet, rowLong, colLong);
    ...
    houseMidpoints.forEach(hm => {
      ...
      const aspectValue = getAspectPercentage(rowPlanet, rowLong, hm.longitude);
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', fontSize: '0.85rem', color: 'var(--text-muted)', justifyContent: 'center', background: 'var(--card-bg)', padding: '1rem', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
          ...
          <div><strong>Full aspect value:</strong> 100</div>
